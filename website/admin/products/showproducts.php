...
<table class="table brand-table">
    ...
    <?php foreach($result as $item):?>
       <tr>
            ...
            <td></td>
            <td width="30%"><img src="../images-products/<?=$item['images']?>"width ="20%"></td>
            <td></td>
            <td><?=$item['status']==1?'Kích hoạt':'<p style="color: red;">Ngưng kích hoạt</p>'?></td>
            <td></td>
            <td><a class="btn btn-sm btn-info" href="?option=productupdate&id=<?=$item['id']?>">update</a>
            <a href="?option=product&id=<?=$item['id']?>" class="btn btn-sm btn-danger" onclick="return confirm('Are you sure?')">delete</a></td>
       </tr> 
        <?php endforeach ;?> 
     </tbody>   
</table>   
